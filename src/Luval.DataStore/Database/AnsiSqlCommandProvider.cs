...
namespace Luval.DataStore.Database
{
    ...
    public class AnsiSqlCommandProvider<TEntity> : IDataCommandProvider<TEntity> where TEntity : class
    {
        ...
        public AnsiSqlCommandProvider(IDataRecordMapper dataRecordMapper)
        {
        ...
        /// <inheritdoc/>
        public IDataCommand GetDelete(TEntity data)
        {
            return GetDelete(GetDataRecord(data)).ToCommand();
        }

        /// <inheritdoc/>
        public IDataCommand GetInsert(TEntity data)
        {
            return GetInsert(_schema, GetDataRecord(data)).ToCommand();
        }

        /// <inheritdoc/>
        public IDataCommand GetUpdate(TEntity data)
        {
            return GetUpdate(GetDataRecord(data)).ToCommand();
        }

        /// <inheritdoc/>
        public IDataCommand Query(Expression<Func<TEntity, bool>> filterExpression, Expression<Func<TEntity, object>> orderByExpression = null, bool descending = false)
        {
